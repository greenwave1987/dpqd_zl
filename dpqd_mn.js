...
//店铺签到
function signCollectGift(token,activity) {
  return new Promise(resolve => {
    const options = {
...
async function readapi(product_id,secret) {
    ...
    for (let i = 0; i < 5; i++) {
        try {
            let {data} = await axios.get(`${new Buffer.from('aHR0cHM6Ly93d3cubXhuenAuY29tL2FwaS9yZW1vdGVfY29uZmlnL2dldD91c2VyX2lkPWxlbnB2dXR4bW1reHJveXImYXBwX2lkPWxlbnB2dXR4bW1reHJveXImYXBwX3NlY3JldD1ZVTlWV1RWTmRVcHpkWGhaYjFkUGEyVlhTMnQzVVQwOQ==', 'base64').toString()}&secret=${secret}&product_id=${product_id}`)
            if(data){
                //console.log(data)
            ...
            console.log(e)
            await $.wait(getRandomNumber(2000, 6000))
        }
    }
